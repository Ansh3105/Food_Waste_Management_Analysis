...
with st.form("add_form"):
    food_id = st.number_input("Food ID", min_value=1)  # Now user sets Food ID manually
    ...
    if submitted:
        if food_id in listing_df["Food_ID"].values:
            st.error(f"❌ Food ID {food_id} already exists! Please use a unique ID.")
        else:
            ...
            st.success(f"✅ New food listing with ID {food_id} added! (Note: not saved to CSV in this demo)")
...
if st.button("Update Quantity"):
    if update_id in listing_df["Food_ID"].values:
...
if st.button("Delete Listing"):
    if delete_id in listing_df["Food_ID"].values:
        ...
        st.error(f"❌ Food ID {delete_id} not found!")
