...
with st.form("add_form"):
    ...
    if submitted:
        if food_id in listing_df["Food_ID"].values:
            ...
            listing_df.to_csv(LISTING_CSV, index=False)  # Save
            st.success(f"✅ Added Food ID {food_id}!")
...
if st.button("Update Quantity"):
    if update_id in listing_df["Food_ID"].values:
        ...
        listing_df.to_csv(LISTING_CSV, index=False)  # Save
        st.success(f"✅ Updated Food ID {update_id}")
    else:
...
if st.button("Delete Listing"):
    if delete_id in listing_df["Food_ID"].values:
        ...
        listing_df.to_csv(LISTING_CSV, index=False)  # Save
        st.warning(f"⚠️ Deleted Food ID {delete_id}")
    else:
        st.error("❌ Food ID not found!")
